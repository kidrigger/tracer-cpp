...
inline std::tuple<vec3, bool> refract(const vec3& incident, const vec3& normal, float ni_over_nr) {
    vec3 uv = incident.normalized();
    float dt = dot(uv, normal);
    float discriminant = 1.0 - ni_over_nr*ni_over_nr * (1 - dt*dt);
    if (discriminant > 0) {
        return std::make_tuple(ni_over_nr * (uv - normal*dt) - normal*sqrt(discriminant), false);
    } else {
